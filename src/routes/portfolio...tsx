...
export const Route = createFileRoute("/portfolio/$slug/$event")({
  loader: ({ params }) => {
    ...
    const eventAlbums = getEventAlbums(params.slug, params.event);
    return { category, event, albums: eventAlbums?.albums ?? [] };
  },
...
function AlbumCard({ album, categorySlug, eventSlug }: { album: Album; categorySlug: string; eventSlug: string }) {
  ...
  return (
    <Link
      ...
      className="group relative overflow-hidden rounded-lg border border-white/10 hover:border-[var(--brand-red)] transition-all duration-300 hover:-translate-y-1"
    ...
    >
      ...
      {coverUrl ? (
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg, oklch(0.24 0.11 240) 0%, oklch(0.18 0.09 230) 60%, oklch(0.20 0.07 220 / 0.7) 100%)",
          }}
        />
      )}
        ...
        <div className="mt-1 flex items-center gap-3">
          ...
          {album.location && (
            <>
              ...
              <span className="font-mono-ui text-[11px] text-white/50">{album.location}</span>
            </>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
          <div className="lg:col-span-8">
            <div className="font-mono-ui text-[10px] uppercase tracking-[0.3em] text-white/40 mb-3">
              {category.name} ·{" "}
              <span style={{ color: "var(--brand-red)" }}>{event.slug}</span>
            </div>
      ...
      <section className="mb-20">
        {albums.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center">
            <div
              className="w-16 h-16 rounded-full border border-white/10 grid place-items-center mb-6"
              style={{ background: "var(--brand-red)" + "15" }}
            >
            ...
            <h2 className="font-display text-3xl text-white/60 mb-2">
              Prossimamente
            </h2>
            <p className="text-white/40 text-sm max-w-sm leading-relaxed">
              Gli album di {event.name.toLowerCase()} saranno pubblicati presto.
              Contattami per prenotare il tuo servizio.
            </p>
            <a
              href="https://wa.me/393332876277"
              target="_blank"
              rel="noopener noreferrer"
              className="mt-8 inline-flex items-center gap-2 rounded-md px-6 py-3 text-sm font-bold uppercase tracking-widest text-white transition-transform hover:scale-[1.03]"
